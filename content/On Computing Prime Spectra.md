...
One important milestone one reaches in learning algebraic geometry is that $\Spec$ is functorial. More specifically, I mean that if we have a ring homomorphism $f: A \to B$, then we get an induced map on the spectra 
$$
f^*: \Spec B \to \Spec A, \qquad \mathfrak{p} \longmapsto f^{-1}(\mathfrak{p})
$$
(be careful here; $\Spec$ is *contravariant!*). This already can get us quite far; it suggests a powerful approach to get a handle on rings we don't know how to work with, by leveraging on ones we do know how to work with. **Explore this more**.
...
\usepackage{tikz-cd}
\begin{document}
\begin{tikzcd} {\operatorname{Spec}\, f(S)^{-1}B} && {\operatorname{Spec}\, B} \\ \\ {\operatorname{Spec}\, S^{-1}A} && {\operatorname{Spec}\, A} \arrow[from=1-1, to=1-3] \arrow[from=1-1, to=3-1] \arrow[from=1-3, to=3-3] \arrow[from=3-1, to=3-3] \end{tikzcd}
\end{document}
```
